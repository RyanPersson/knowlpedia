...
**Proof sketch**: Present \(M\) as the {{< knowl id="cokernel-module" text="cokernel" >}} of a homomorphism \(R^m\to R^n\), represented by a matrix over \(R\). Apply Smith normal form to diagonalize that matrix up to invertible row/column operations; the resulting diagonal entries \(d_i\) yield the cyclic summands \(R/(d_i)\), and the number of zero diagonal entries yields the free rank \(r\).
