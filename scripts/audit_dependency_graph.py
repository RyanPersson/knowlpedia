...
def audit(package_dir: Path, profile_name: str) -> dict[str, object]:
    ...
    duplicate_ids = sorted({knowl.id for knowl in knowls if sum(item.id == knowl.id for item in knowls) > 1})
    if duplicate_ids:
        raise ValueError("Duplicate knowl ids: " + ", ".join(duplicate_ids))
    registry = {knowl.id: knowl for knowl in knowls}
    prerequisites = {
        knowl_id: [compiler.split_target(target)[0] for target in knowl.prerequisites
                   if compiler.split_target(target)[0] in registry]
        for knowl_id, knowl in registry.items()
    }
    missing = sorted({
        target for knowl in registry.values() for target in knowl.prerequisites
        if compiler.split_target(target)[0] not in registry
    })
