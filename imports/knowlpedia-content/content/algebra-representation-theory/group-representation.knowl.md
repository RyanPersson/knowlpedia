+++
id = "algebra-representation-theory/group-representation"
...
- \(V\) is a finite-dimensional [[linear-algebra/vector-space|vector space]] over \(k\), and
...
A representation \((V,\rho)\) is equivalently a left module over the [[algebra-representation-theory/group-algebra|group algebra]] \(k[G]\): extend \(\rho\) \(k\)-linearly to an algebra homomorphism
\[
k[G]\longrightarrow \mathrm{End}_k(V),
\]
where \(\mathrm{End}_k(V)\) consists of [[linear-algebra/linear-map|linear maps]] \(V\to V\).
...
\]
Equivalently, \(f\) is a \(k[G]\)-module homomorphism (compare [[algebra-modules/module-homomorphism|module homomorphism]]).

### Character
To any representation one associates its [[algebra-representation-theory/character|character]] \(\chi_\rho(g)=\mathrm{tr}(\rho(g))\), using the [[linear-algebra/trace|trace]].
...
See also: [[algebra-representation-theory/subrepresentation|subrepresentation]], [[algebra-representation-theory/irreducible-representation|irreducible representation]], [[algebra-representation-theory/regular-representation|regular representation]].
