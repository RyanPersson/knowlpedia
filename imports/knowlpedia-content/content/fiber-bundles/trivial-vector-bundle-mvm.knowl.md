+++
id = "fiber-bundles/trivial-vector-bundle-mvm"
...
Let $M$ be a [[fiber-bundles/smooth-manifold|smooth manifold]] and let $V$ be a finite-dimensional real vector space.
...
The trivial bundle is the vector-bundle analogue of the [[fiber-bundles/trivial-principal-bundle-mgm|trivial principal bundle]].

## Examples
1. **Euclidean tangent bundle.**
   The [[fiber-bundles/tangent-bundle|tangent bundle]] of $\mathbb R^n$ is trivial: $T\mathbb R^n\cong \mathbb R^n\times \mathbb R^n$ using the standard coordinate vector fields.
...
3. **Connections on trivial bundles.**
   A [[fiber-bundles/connection-on-a-vector-bundle|connection on a vector bundle]] on $M\times V$ can be written globally in a chosen basis as $\nabla=d+A$, where $A$ is a matrix of 1-forms on $M$.
