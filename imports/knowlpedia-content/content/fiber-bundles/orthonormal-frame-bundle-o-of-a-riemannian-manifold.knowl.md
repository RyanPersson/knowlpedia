+++
id = "fiber-bundles/orthonormal-frame-bundle-o-of-a-riemannian-manifold"
title = "Orthonormal frame bundle"
kind = "knowl"
summary = "Principal O(n) subbundle of the frame bundle determined by a Riemannian metric."
...
\]
The structure group is the orthogonal group $\mathrm{O}(n)$ (a [[fiber-bundles/lie-group|Lie group]]), acting on the right by change of orthonormal basis.

With this action, $O(TM)\to M$ is a principal $\mathrm{O}(n)$-bundle and is a reduction of $\mathrm{Fr}(TM)\to M$ from $\mathrm{GL}(n)$ to $\mathrm{O}(n)$; this reduction is the basic example in [[fiber-bundles/example-reduction-of-gl-structure-to-o-using-a-bundle-metric|reducing structure group using a bundle metric]].
