+++
id = "fiber-bundles/cartans-first-structure-equation"
...
Given a [[fiber-bundles/principal-connection|principal connection]] $\omega\in\Omega^1(F(M);\mathfrak{gl}(n,\mathbb{R}))$, define the **torsion $2$-form** $\Theta\in\Omega^2(F(M);\mathbb{R}^n)$ by the covariant exterior derivative of $\theta$:
\[
\Theta \;:=\; D\theta.
\]
...
T_\nabla(X,Y)=\nabla_XY-\nabla_YX-[X,Y],
\]
where $[X,Y]$ is the [[fiber-bundles/lie-bracket|Lie bracket]] of vector fields. In particular, $\nabla$ is torsion-free if and only if $\Theta=0$, equivalently
\[
