...
Nakayama-style arguments convert statements “mod the maximal ideal” into statements in the whole module. The results below are standard consequences of [[algebra-commutative/nakayama-lemma|Nakayama's lemma]].

## Corollary (lifting generators from the residue field)
Let \((R,\mathfrak m)\) be a [[algebra-commutative/local-ring|local ring]] with maximal ideal \(\mathfrak m\) (as in [[algebra-commutative/maximal-ideal-local-ring|maximal-ideal characterization]]), and let \(M\) be a finitely generated \(R\)-module. Let \(k=R/\mathfrak m\) be the [[algebra-commutative/residue-field|residue field]], so \(M/\mathfrak m M\) is naturally a \(k\)-[[linear-algebra/vector-space|vector space]].
...
3. **Minimal number of generators.**
   The minimal number of \(R\)-module generators of \(M\) equals \(\dim_k(M/\mathfrak m M)\). In particular, \(M\) is [[algebra-modules/cyclic-module|cyclic]] if and only if \(\dim_k(M/\mathfrak m M)=1\).
