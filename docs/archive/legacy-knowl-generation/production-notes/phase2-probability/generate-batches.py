#!/usr/bin/env python3
"""Generate batch prompts for probability section."""
...
num_batches = 10
batch_size = (len(slugs) + num_batches - 1) // num_batches
...
all_slugs_str = ", ".join(slugs)

for i, batch in enumerate(batches, 1):
    batch_slugs = "\n".join(f"- {s}" for s in batch)
    prompt = prompt_template.format(
        all_slugs=all_slugs_str,
        batch_slugs=batch_slugs
    )

    with open(f"/Users/ryanpersson/anki/code/blog/tmp/phase2-probability/batch{i}.md", "w") as f:
        f.write(prompt)

    print(f"Batch {i}: {len(batch)} slugs - {', '.join(batch)}")

# Create config file
with open("/Users/ryanpersson/anki/code/blog/tmp/phase2-probability/oracle-config.txt", "w") as f:
    for i in range(1, len(batches) + 1):
        f.write(f"/Users/ryanpersson/anki/code/blog/tmp/phase2-probability/batch{i}.md\n")

print(f"\nCreated {len(batches)} batch files and oracle-config.txt")
