...
# Build cross-reference string for other topology sections
def get_other_topology_slugs(current_section):
    result = []
    for section, slugs in topology_sections.items():
        if section != current_section:
            result.append(f"**{section}:** {', '.join(slugs)}")
    return "\n\n".join(result)
...
# Create batch files
batch_num = 1
for section_name, slugs in topology_sections.items():
    other_topology = get_other_topology_slugs(section_name)
    
    batch_slugs = "\n".join(f"- {s}" for s in slugs)
    current_slugs = ", ".join(slugs)
    
    content = prompt_template.format(
        section_name=section_name,
        current_slugs=current_slugs,
        other_topology_slugs=other_topology,
        shared_foundations=shared_foundations_slugs,
        linear_algebra=linear_algebra_slugs,
...
config_lines = []
for i in range(1, 7):
    config_lines.append(f"-pf {output_dir}/batch{i}.md")
